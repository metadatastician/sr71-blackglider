...
# entirely, so nobody noticed that .github/settings.yml shipped `name: "sr71-blackglider"`
...
# an instantiation. The template submitted the literal `sr71-blackglider` as its own
...
# is set to `owner/sr71-blackglider` and matches — but wrong anywhere the
...
# deliberate placeholder files as a failure. A clone into `sr71-blackglider-2`,
...
if [ -z "$REPO_NAME" ]; then
    # `git config --get` exits 1 when the key is unset; under `set -e -o
    # pipefail` that killed the whole gate silently in any remoteless checkout
    # (exit 1, zero output, before a single file was scanned). `|| true` keeps
    # the promised fallback-to-basename reachable.
