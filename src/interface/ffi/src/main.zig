...
// SR71_BLACKGLIDER FFI Implementation
//
...
const VERSION = "0.1.0";
const BUILD_INFO = "SR71_BLACKGLIDER built with Zig " ++ @import("builtin").zig_version_string;
...
test "lifecycle" {
    const handle = sr71_blackglider_init() orelse return error.InitFailed;
    defer sr71_blackglider_free(handle);

    try std.testing.expect(sr71_blackglider_is_initialized(handle) == 1);
}

test "error handling" {
    const result = sr71_blackglider_process(null, 0);
    try std.testing.expectEqual(Result.null_pointer, result);

    const err = sr71_blackglider_last_error();
    try std.testing.expect(err != null);
}

test "version" {
    const ver = sr71_blackglider_version();
