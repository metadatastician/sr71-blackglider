...
# SR71_BLACKGLIDER ABI/FFI Documentation
...
│  generated/abi/sr71_blackglider.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libsr71_blackglider.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── sr71_blackglider.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── sr71_blackglider.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/sr71_blackglider.h
```
...
```c
#include "sr71_blackglider.h"

int main() {
    void* handle = sr71_blackglider_init();
    if (!handle) return 1;

    int result = sr71_blackglider_process(handle, 42);
    if (result != 0) {
        const char* err = sr71_blackglider_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    sr71_blackglider_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lsr71_blackglider -L./zig-out/lib
```

### From Idris2

```idris
import SR71_BLACKGLIDER.ABI.Foreign
...
```rust
#[link(name = "sr71_blackglider")]
extern "C" {
    fn sr71_blackglider_init() -> *mut std::ffi::c_void;
    fn sr71_blackglider_free(handle: *mut std::ffi::c_void);
    fn sr71_blackglider_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = sr71_blackglider_init();
        assert!(!handle.is_null());

        let result = sr71_blackglider_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libsr71_blackglider = "libsr71_blackglider"

function init()
    handle = ccall((:sr71_blackglider_init, libsr71_blackglider), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:sr71_blackglider_process, libsr71_blackglider), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:sr71_blackglider_free, libsr71_blackglider), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/sr71_blackglider.h
   ```
